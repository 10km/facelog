...
public class Server implements ServiceConstant {
	public static class Builder {
		...
		private int servicePort = CONFIG.getInt(NET_PORT, DEFAULT_PORT);
	...
	public Server(List<?> services, int servicePort, ExecutorService executor) {
		...
		this.servicePort = servicePort;
		this.executor = checkNotNull(executor);
		processor = new ThriftServiceProcessor(new ThriftCodecManager(), ImmutableList.<ThriftEventHandler>of(),
				services);
		ThriftServerDef serverDef = ThriftServerDef.newBuilder().listen(servicePort).withProcessor(processor)
				.using(this.executor).build();
		NettyServerConfig serverConfig = NettyServerConfig.newBuilder().setBossThreadExecutor(this.executor)
				.setWorkerThreadExecutor(this.executor).build();
