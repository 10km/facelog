...
public class GenericUtils {
     ...
     */
    public static final byte[] toBytes(ByteString buffer){
        if(null == buffer){
            return null;
        }
    ...
    private static <T> InputStream getInputStream(T src) throws IOException {
        ...
        if (src instanceof InputStream){
            ...
            return new ByteArrayInputStream(toBytes((ByteString) src));
    ...
    private static final CollectionUtils.DualTransformer<ByteString,byte[]> BYTE_BUFFER_DUAL_TRANSFORMER = new CollectionUtils.DualTransformer<ByteString,byte[]>(){
        @Override
        public byte[] toRight(ByteString input) {
            return toBytes(input);
        }
        @Override
        public ByteString fromRight(byte[] input) {
            return null == input ? null : ByteString.of(input);
        }};
    /** get a view of {@code Map<ByteString,V>} with {@code byte[]} key type */
    public static final<V>Map<byte[],V> toBytesKey(Map<ByteString,V> source){
        if(null == source){
            return null;
        }
        return CollectionUtils.tranformKeys(source, BYTE_BUFFER_DUAL_TRANSFORMER);
    }
    private static final EntryTransformer<Object,ByteString,byte[]> BYTE_BUFFER_TRANSFORMER = new EntryTransformer<Object,ByteString,byte[]>(){
        @Override
        public byte[] transformEntry(Object key, ByteString value) {
             return BYTE_BUFFER_DUAL_TRANSFORMER.toRight(value);
        }};
    /** get a view of {@code Map<K,ByteString>} with {@code byte[]} value type */
    public static final<K> Map<K,byte[]> toBytesValue(Map<K,ByteString> source){
        if(null == source){
            return null;
        }
        return Maps.transformEntries(source, BYTE_BUFFER_TRANSFORMER);
    }
    private static final Function<ByteString,byte[]> TO_RIGHT_BYTE_BUFFER = new Function<ByteString,byte[]>(){
        @Override
    ...
    public static final List<byte[]> toBytes(List<ByteString>source){
        if(null == source){
            return null;
        }
    ...
    public static final Collection<byte[]> toBytes(Collection<ByteString>source){
        if(null == source){
            return null;
        }
