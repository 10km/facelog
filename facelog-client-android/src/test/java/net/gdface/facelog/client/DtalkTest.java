...
//		rootToken = facelogClient.applyRootToken("root", false);
	...
	@Test
	public void testFacelogConnect() {
		boolean localhosthConn = ClientFactory.builder().setHostAndPort("127.0.0.1", DEFAULT_PORT).testConnect();
		logger.info("localhosthConn={}",localhosthConn);
		boolean localConn = ClientFactory.builder().setHostAndPort("localfacelog", DEFAULT_PORT).testConnect();
		logger.info("localConn={}",localConn);
		boolean cloudConn = ClientFactory.builder().setHostAndPort("facelog.gdface.net", DEFAULT_PORT).testConnect();
		logger.info("cloudConn={}",cloudConn);
	}
